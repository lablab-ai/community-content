...
# Building AI Trading Agents with On-Chain Identity

## Introduction

This tutorial walks you through a complete, reusable AI trading agent template with four core layers:

- **On-chain identity** via ERC-8004 Agent Registry (Sepolia)
- **Trade execution** via Kraken CLI (paper trading supported)
- **Capital management** via Hackathon Vault + Risk Router contracts
- **Cryptographic explainability** via EIP-712 signed checkpoints

Any team can pick this up, swap in their own model or strategy, and run it. The identity, risk, and audit layers stay the same — only the strategy changes.
...
This section covers the first question. Part 6 covers the second.
...
| Where | What you see |
|-------|-------------|
| **Terminal** | Printed immediately after each decision via `formatExplanation()` |
| **Terminal (checkpoint block)** | Printed again inside the signed checkpoint summary via `formatCheckpointLog()` |
| **EIP-712 signature** | `keccak256(reasoning)` is computed and included in the signed payload |
| **`checkpoints.jsonl`** | Full reasoning string appended as JSON after every tick |
| **Dashboard** | Shown on each checkpoint card under the action badge |

You'll see the first output in the terminal **within 30 seconds** of starting `npm run run-agent` (the agent warms up for 5 ticks collecting price samples, then starts making decisions).
...
The demo runs `MomentumStrategy` — no LLM involved. Reasoning is generated directly from price arithmetic. That's intentional: the template works out of the box without any API keys beyond Kraken.
...
Uncomment and fill in your client. The key constraint: **reasoning must reference actual market data values** (price, volume, VWAP). This makes the explanation auditable — anyone can cross-check the claim against historical market data.

### Reading a full tick in the terminal

Here's what one complete tick looks like:
...
[2026-03-28T12:27:05.553Z] HOLD XBTUSD @ $66,391.20  ← formatExplanation() output
  Confidence: 50%
  Reason: No clear momentum (-0.00% change).
          Holding current position.
  Market: bid=66391.2, ask=66391.3,
...
CHECKPOINT — HOLD XBTUSD                            ← formatCheckpointLog() output
  ...
  Price:     $66391.2
  Confidence: 50%
  Reasoning: No clear momentum (-0.00% change).
             Holding current position.
  Sig:       0x009aa74a5314926499...0d7940931c       ← EIP-712 signature
  Signer:    0x13Ef924EB7408e90278B86b659960AFb00DDae61
────────────────────────────────────────────────────────────────────────

[agent] Checkpoint posted to ValidationRegistry: 0xca62bb0d47f2b53a1a...
```

The sequence within each tick:
1. `kraken.getTicker()` — fetch live price from Kraken
2. `strategy.analyze(market)` — strategy returns a `TradeDecision` (including `reasoning`)
3. `formatExplanation()` — prints the human-readable summary
4. `generateCheckpoint()` — signs the decision with EIP-712; reasoning is hashed into the signature
5. `formatCheckpointLog()` — prints the signed checkpoint block
6. ValidationRegistry post — checkpoint hash submitted on-chain to Sepolia
7. `fs.appendFileSync()` — checkpoint appended to `checkpoints.jsonl`
...
- The hash is a commitment: if you have the hash and the original string, you can verify they match via `verifyReasoningIntegrity()`
...
1. Clone the repo: `git clone https://github.com/Stephen-Kimoi/ai-trading-agent-template`
2. `npm install`
...
You now have a production-ready AI trading agent with:
- **On-chain identity** via ERC-8004 (ERC-721 token) — verifiable, transferable, composable
- **Risk controls** via the RiskRouter — every trade validated against defined limits before execution
- **Cryptographic explainability** via EIP-712 checkpoints — every decision signed, hashed, and auditable
- **Live dashboard** — full visibility into agent behavior in real time

The key insight is the separation of concerns: the strategy layer (your code) is fully decoupled from the infrastructure layer (identity, risk, exchange, checkpoints). Swap in any model — Claude, Llama, Groq, or a simple algorithm — and the rest runs unchanged.

Explore the full source at [github.com/Stephen-Kimoi/ai-trading-agent-template](https://github.com/Stephen-Kimoi/ai-trading-agent-template).
