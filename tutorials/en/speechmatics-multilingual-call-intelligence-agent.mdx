...
description: "Build a multilingual AI hackathon project with Speechmatics: real-time translation into five languages, plus a live Gemini summary and sentiment layer."
...
**Q: Why can't Speechmatics generate the live summary and sentiment natively in real time?**
Speechmatics' own summarization and sentiment features only run in batch mode, over a completed transcript. There's no `summarization_config` for a live session — the rolling summary in this tutorial is a pattern built on top of the real-time transcript using an external LLM (Gemini here), not a native Speechmatics real-time API.

**Q: Can Speechmatics really translate one audio stream into all five languages at the same time?**
Yes — `TranslationConfig(target_languages=[...])` accepts up to five target languages in a single real-time session, and the server streams back a separate `AddTranslation` message per language as the same audio is transcribed.

**Q: What are some AI hackathon project ideas using Speechmatics?**
Beyond call intelligence, Speechmatics' real-time and translation APIs fit well into voice agents, multilingual meeting assistants, live-captioned support widgets, and accessibility tools — all common project categories in online AI hackathons.

**Q: How can I use this multilingual call intelligence agent in an AI hackathon?**
The companion repo is a working starting point: fork it, swap the sample call for a live microphone or a real recording pipeline, and extend the analysis layer (per-speaker sentiment, persisted call history, alerting) to fit whatever track or use case your hackathon is judging.
