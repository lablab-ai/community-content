...
To illustrate how **AgentOps** enhances AI agent monitoring, we’ll build upon the multi-agent system created in my previous tutorial on [CrewAI Multi-Agent System](https://lablab.ai/ai-tutorials/crewai-multi-agent-system). In that tutorial, we developed a complex system involving multiple AI agents, each handling different roles like data retrieval, customer support, and quality assurance.
