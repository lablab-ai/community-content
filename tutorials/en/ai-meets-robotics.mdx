...
<Img src="https://res.cloudinary.com/dygkv9gam/image/upload/v1768198365/open-sample-world_lmfpm0.png" alt="Sample world menu" caption="Files -> Sample World" />

<Img src="https://res.cloudinary.com/dygkv9gam/image/upload/v1768198365/panda-sample-world_xtmbxk.png" alt="Open Sample world" />

<Img src="https://res.cloudinary.com/dygkv9gam/image/upload/v1768198365/opened-panda-sample_e8qlac.png" alt="Opened Panda Sample" />

### Creating the Controller Setup (macOS)

To ensure the robot "brain" is always available in the simulator's internal library, we use a manual injection method via the Terminal.

**Create the Controller Folder:** Open your Terminal and run the following command to create a dedicated space for your AI script inside the Webots application:

```bash
sudo mkdir -p /Applications/Webots.app/Contents/projects/robots/franka_emika/panda/controllers/llm_robot_planner
```

**Initialize the Script:** Create your Python file (`llm_robot_planner.py`) and move it into that directory.

**Set Permissions:** Ensure Webots has permission to execute your code:

```bash
sudo chmod -R 777 /Applications/Webots.app/Contents/projects/robots/franka_emika/panda/controllers/llm_robot_planner
```

**Create the Initial Controller Code:** Open the python code (`llm_robot_planner.py`) via your preferred IDE or terminal and insert this code:
