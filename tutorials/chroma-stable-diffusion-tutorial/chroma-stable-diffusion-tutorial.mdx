...
- Part 1 - Getting prompt for Stable Diffusion. In this part, we will go through Chroma DB and Cohere LLM. We will load the document, split it into smaller chunks, embed them using Cohere and then we will use Chroma to query the database and get the prompt for Image Generation.
...
To install the necessary libraries wich we gonna use throughout the tutorial:
...
Firstly, let's quickly upload PDF file to Google Colab. In this tutorial, we will go with `Harry Potter and the Sorcerer's Stone` book. You can download the PDF file [here](https://www.pdfdrive.com/the-complete-harry-potter-harry-potter-and-the-sorcerers-stone-harry-potter-and-the-chamber-of-secrets-harry-potter-and-the-prisoner-of-azkaban-harry-potter-and-the-goblet-of-fire-harry-potter-and-the-order-of-the-phoenix-h-d157198854.html). 

After, downloading back to Google Colab and go to `Files` tab on the left side of the screen, click `Upload to session storage` and upload the file. Wait until the file is uploaded and then `copy` the path to the file.
...
Why? We should to make sure that the LLM can process the document. If the document is too long, the LLM will not be able to process it. We will use `RecursiveCharacterTextSplitter` to split the document into smaller chunks. We will split the document into chunks of 1000 characters with 0 overlap.
...
Perfect! We have done with chain. Now, we can query based on the processed file. Let's try to ask a about `Harry Potter`.
...
You should see something like below image, but don't worry if you see something different.
...
Now, you can download the image or see the image instantly.
...
Congratulations! You've successfully generated an image using Stable Diffusion with the prompt generated by Chroma based on the Cohere embeddings.
