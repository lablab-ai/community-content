...
Let's change directory to our front-end project again. If the setup process from earlier using `create-react-app` is done successfully, we should have `App.js` file ready to edit. This is the only file we will edit to build our user interface, thanks to TailwindCSS which already took care of our styling, all in one place.
### App.js ⚛️
...
Phew, now, if everything goes well, this is how our `App.js` will look like:
