...
def completion(prompt: str):
    ...
    promptTemplate = PromptTemplate(
        ...
        template="You are a helpfull assistant. {prompt}",
    )

    anthropic = ChatAnthropic(anthropic_api_key=f"{os.getenv('ANTHROPIC_API_KEY')}")
...
```json
{"prompt": "What is a good name for a company that makes colorful socks", "response": "Here are some suggestions for names for a company that makes colorful socks:\n\n• BrightSocks\n\n• Colorful Toes \n\n• Rainbow Socks\n\n• Happy Feet Socks\n\n• Sock It To Me \n\n• Joy of Socks\n\n• Sock Pop \n\n• FunSocks\n\n• Lively Legwear\n\n• ChromaSocks   (Chroma meaning color in Greek)\n\nThese names convey brightness, color, fun and whimsy which seems well suited for a colorful sock company.  You can of course adapt these by combining words or coming up with your own variations that capture the essence of a colorful, fun sock brand."}
```
...
const App = () => {
  ...
  const completion = async () => {
    ...
    axios.get(`http://0.0.0.1:8000/api/chat/${input}`)
      .then(res => {
...
Go to `https://gist.github.com/abdibrokhim/6c9ba7e4e7f8866b110067aaf009bfa7` and copy/paste into your `styles.css` file. Then, import `styles.css` into `index.js` file:
...
Let's try to chat with the AI assistant!

<Img src="./images/chat1.png" alt="Chat Preview" caption="Chat Preview"/>

<Img src="./images/chat2.png" alt="Chat Preview" caption="Chat Preview"/>
