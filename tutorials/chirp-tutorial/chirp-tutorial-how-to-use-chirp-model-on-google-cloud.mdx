...
By default API version is set to `V1`, change it to `V2`, specify the language `English - United States` in our case spoken in the audio file from the `dropdown menu`, then choose `Chirp` as transcription model and our newly created `chirp-recognizer` as the recognizer.
...
<Img src="./images/transcribedText.png" alt="Google Cloud console transcription text" caption="Google Cloud console transcription text"/>
