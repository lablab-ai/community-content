---
title: "What are Stable Diffusion models?"
description: "Venture into the enigmatic world of Stable Diffusion models, uncovering their intricacies, versatile applications, and the compelling reasons to adopt them in your projects!"
image: "https://imagedelivery.net/K11gkZF3xaVyYzFESMdWIQ/b92d86eb-babb-4f4a-4932-8ca4929b4a00/full"
authorUsername: "Marek"
---

## What are Stable Diffusion models?

Latent diffusion models (LDMs) are a type of image generation technique that work by iteratively "de-noising" data in a latent representation space, and then decoding the representation into a full image. This is in contrast to other popular image synthesis methods such as generative adversarial networks (GANs) and the auto-regressive technique used by DALL-E. The Stable Diffusion model is created by a collaboration between engineers and researchers from [CompVis](https://huggingface.co/CompVis), [Stability AI](https://stability.ai/), and [LAION](https://laion.ai/) and released under a [Creative ML OpenRAIL-M](https://huggingface.co/spaces/CompVis/stable-diffusion-license) license, which means that it can be used for commercial and non-commercial purposes. The release of this file is the culmination of many hours of collective effort to compress the visual information of humanity into a few gigabytes. Furthermore, the model also supports image-to-image style transfer, as well as upscaling and generating images from a simple sketch. Included is also an AI-based Safety Classifier, which understands concepts and other factors in generations to remove outputs that may not be desired by the model user.

## Where can Stable Diffusion Models be used and why?

Stable Diffusion is a latent diffusion model that is capable of generating detailed images from text descriptions. It can also be used for tasks such as inpainting, outpainting, text-to-image and image-to-image translations.

Due to its powerful ability to generate images based on different types of prompts (text, image, etc.) actually only our imagination limits us where this tool can be used. It can fasten or enhance our ability to create more engaging content for our customers or users in general. Even if you’re a creative who sucks with a creative block you can use generated images to get inspired and overcome the stagnation. We encourage you to do a unique art with usage of Stable Diffusion models or first drafts for your artistic (or everyday) journey.

## Examples of technologies that use Stable Diffusion models

Already there are dozens of tools using Stable Diffusion models. We want to inspire you and show how amazing aps have been created during our Hackathons, so we gathered them in one place!

### 1. Stable Diffusion Creator Tool
Lets you create videos based off stable key frames and interpolate the results using custom apis.

[Creator Tool](https://lablab.ai/event/stable-diffusion-hackathon/Fast%20Path/Stable%20Diffusion%20Creator%20Tool)
### 2. Forensic ai
AI Portrait Generator, where you can change the input data to generate realistic portraits.

[forensic AI](https://lablab.ai/event/stable-diffusion-hackathon/SD4FUN/forensic%20ai)
### 3. Talk In threeD
Provides near real time (less than 20 secs) text to 3d model generation.

[Talk In threeD](https://lablab.ai/event/stable-diffusion-hackathon/Metaverse-Streamer/Talk%20In%20threeD%20)
### 4. Fast Ads
An AI tool that will automate this online Ad creation process.

[Fast Ads](https://lablab.ai/event/stable-diffusion-hackathon/Advert-AI/Fast%20Ads)
### 5. Same Same But Different
Without the risk of running into copyright or plagiarism laws, SSBD aims to provide a safe space to get similar-styled images with full originality.

[SSBD](https://lablab.ai/event/stable-diffusion-hackathon/DDC/Same%20Same%20But%20Different)
### 6. Stable CAD Generator
Solves the design problem of many people who work in engineering and architecture, even though our main focus is on CADs.

[Stable CAD Generator](https://lablab.ai/event/stable-diffusion-hackathon/Chuck%20Norris/Stable%20CAD%20Generator)

### 7. Dreamixer
A Stable Diffusion based AI tool to generate comic strips from text.

[Dreamixer](https://lablab.ai/event/stable-diffusion-hackathon/Morpheus/Dreamixer)
### 8. AI and early child development
A solution that can create custom images for storytelling based on the unique characteristics of the child to empower the educational system.

[AIECD](https://lablab.ai/event/stable-diffusion-hackathon/Xena/AI%20and%20early%20child%20development)
### 9. Novel view generation
Image-to-image translation with an additional parameter of an object from an unrecognizable angle.

[Novel View Generation](https://lablab.ai/event/stable-diffusion-hackathon/Aqua%20learning/Novel%20view%20generation)
### 10. Show me a Story
Generates a story text, from GPT-2, and creates a visual image of the story.

[Show me a Story](https://lablab.ai/event/stable-diffusion-hackathon/AI4Lokal/Show%20me%20a%20Story)
### 11. Immersive Books
provides book readers a more immersive experience.

[Immersive Books](https://lablab.ai/event/stable-diffusion-hackathon/The%20Prompt%20Engineers/Immersive%20Books)
### 12. Chrolove
Pictures generated by the Stable Diffusion are used for mental health therapeutic purposes.

[Chrolove](https://lablab.ai/event/stable-diffusion-hackathon/Accelerate%20Mental%20Health%20Engineering/Chrolove)

### 13. TEXT TO HUMAN AI
A Text-to-Video Human avatar generator.

[TTHAI](https://lablab.ai/event/stable-diffusion-hackathon/VISIO/TEXT%20TO%20HUMAN%20AI)
### 14. Dimensational
Atext-to-image topographic texture generator.

[Dimensational](https://lablab.ai/event/stable-diffusion-hackathon/Dimensational/Dimensational)
### 15. A Machine Making a Human
Make prompts more in line with what the user likes.

[AMMAH](https://lablab.ai/event/stable-diffusion-hackathon/Team%20Get%20It%20Done/A%20machine%20making%20a%20human)

## So how can I start using Stable Diffusion models?

<Img src="https://imagedelivery.net/K11gkZF3xaVyYzFESMdWIQ/f51e5c22-d375-4941-632a-dfa5942b3f00/full" alt="a boy with laptop under the tree" />

If you don't know how to adopt them, we recommend you to first of all visit our [Stable Diffusion tutorials](https://lablab.ai/tech/stable-diffusion). From that place the journey is really simple, as you already know how such models can be used, so it’s time for practice. And where better to test your skills than during 3 or 7 days Hackathons? With a group of like-minded people and with assistance of mentors? I can’t come up with a better place, so start (or improve) your AI journey with New Year’s resolutions and with lablab.ai’s [AI Hackathons](https://lablab.ai/event)!
