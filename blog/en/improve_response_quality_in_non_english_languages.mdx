...
def translate_prompt_from_language_x_to_english(prompt_text_from_user, language):
    translator = Translator()
    translated_text = translator.translate(prompt_text_from_user, dest='en', src=language).text
...
def send_translated_english_prompt_to_gpt3(prompt_text):
    llmGPT = OpenAI(openai_api_key=<Your OPEN_AI_KEY>, model_name= "gpt-3.5-turbo")
    prompt = PromptTemplate(template=prompt_text, input_variables=["text"])
    llm_chain = LLMChain(prompt=prompt, llm=llmGPT)
    ## Take a deep breath is a recommended prompt engineering technique   
    english_response = llm_chain.run(text="Take a deep breath") 
    return english_response
```
### method to translate english response from GPT To original input language
```python

def translate_english_response_to_input_language_x(gpt_response_in_english, language):
    translator = Translator()
    translated_response = translator.translate(gpt_response_in_english, dest=language, src='en').text
    return translated_response

```

There are two options to achieve the translation. One option is to use Google Translate API while other option is to use GPT 3.5 itself to translate.
